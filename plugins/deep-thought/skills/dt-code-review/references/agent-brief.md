...
Send this to the agent unchanged. Use a general-purpose agent unless the environment offers a
read-only reviewer.

The model is the session default unless you pin one. Pin `opus` or `fable` with the Agent tool's
`model` parameter when the change deserves the stronger reader. `dt-overkill-code-review` is what runs
three different models against each other.

It is the same brief `dt-overkill-code-review` sends to each of its three agents, so a review from
either skill can be read the same way.
